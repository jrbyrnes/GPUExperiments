...
string kernelStr      =
    ...
    "			 int readIterations)\n"
    "{\n"
    "   __local float localData[512];\n"
    "   uint gid = get_global_id(0);\n"
    "   localData[gid] = gid;\n"
    "   for (int j = 0; j < writeLoops; j++) {\n"
    "   	for (int i = gid; i < writeIterations; i++) {\n"
    "			localData[i] = localData[i-1] * 2;\n"
    "   	}\n"
    "   }\n"
    "   for (int i = 0; i < readIterations; i++) {\n"
    "     y[gid] = localData[gid+writeIterations-1] * x[gid] + y[gid];\n"
    "   }\n"
    "}\n";
...
int main(int argc, char * argv[])
{
  ...
  try
  {
    ...
    kernel.setArg(2, a);
    kernel.setArg(3, 16384/ldsSize);
    kernel.setArg(4, ldsSize/256);
    kernel.setArg(5, nIterations);
